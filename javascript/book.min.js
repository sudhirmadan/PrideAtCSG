...
var VERSION_NAME="6.4.9",BUILD_DATE="2025071701",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8,documentView_book:9,singleDocumentView_book:10,card_book:11},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},singleBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},bookCorner={top_left:0,top_right:1,bottom_left:2,bottom_right:3},BrowserType=
...
c.left-c.right-b)/2+c.left,y:(windowHeight-c.top-c.bottom-d)/2+c.top}},scalePages:function(b,c,d,e,f,g,h,k,l){try{var n=1,p=1;n=$.easing.easeInOutSine(null,c,1,f.x/e.x-1,d);p=$.easing.easeInOutSine(null,c,1,f.y/e.y-1,d);for(c=0;c<b.length;c++){var t=this.pageArray[b[c]];if(t){var B=transformCSS({tran:l?l:{x:0,y:0},scale:{x:n,y:p},origin:g});this.isCoverPage(b[c])&&(t.tran=l?l:{x:0,y:0},t.scale={x:n,y:p},t.rotateY?(t.$page[0].style.removeProperty("transform-origin"),d=void 0,B=transformCSS({scale:{x:n,
y:p},perspective:6E3,rotateY:null!=(d=t.rotateY)?d:0})):(d=void 0,B=transformCSS({origin:g,scale:{x:n,y:p},perspective:6E3,rotateY:null!=(d=t.rotateY)?d:0})));t.setCSSByStyle(B);if(t.page.pageFlipShadow){var v=h?transformCSS({scale:{x:k.x/n,y:1},origin:g}):transformCSS({scale:{x:1,y:1},origin:g});t.page.pageFlipShadow&&t.page.pageFlipShadow.setCSSByStyle(v)}}}return{x:n,y:p}}catch(w){}},getTranScaleByIndex:function(b){return this.getFitWHByIndex(b)},getFitWHByIndex:function(b,c,d){this.getSpace(!1);
...
this.initAttachments();this.initEvent()},defineProperty:function(){this.showPage=this.flipBookElement=this.bookAttachments=null;this.scrollableWidth=this.contentWidth=0;this.pageSpacing=bookConfig.continuousPageArrangement?0:windowWidth;this.dragStartOffset=this.currentOffset=this.totalOffset=this.startOffset=0;this.swipeSpeed=this.panSpeed=this.scrollSpeed=1;this.unitOffset=0;this.unitRotate=-45;this.k=0;this.isNextPage=!0;this.flipAnimation=null;this.flipping=!1;this.isUpdateOffset=!0;this.status_resize=
this.isZooming=this.status_zoom=this.selected=!1;this.toPageIndex=this.dMouseWheelTime=0},getRealBookIndex:function(b){return b=Number.between(b,1,bookConfig.totalPageCount)},initBook:function(){this.flipBookElement=$("<div class='card-book' id='card-book'></div>");this.parent.append(this.flipBookElement);this.layoutPages("init")},layoutPages:function(b){var c=bookConfig.topMargin+templateArea.getTopHeight(),d=bookConfig.bottomMargin+templateArea.getBottomHeight(),e=bookConfig.leftMargin+templateArea.getLeftWidth(),
f=bookConfig.rightMargin+templateArea.getRightWidth();f=windowWidth-e-f;d=windowHeight-c-d;var g=bookConfig.largePageWidth*(-parseFloat(bookConfig.visibleAreaLeft)+parseFloat(bookConfig.visibleAreaRight))/100,h=bookConfig.largePageHeight*(parseFloat(bookConfig.visibleAreaBottom)-parseFloat(bookConfig.visibleAreaTop))/100;if(bookConfig.continuousPageArrangement)this.width=f,this.height=d,this.bookLeft=e,this.bookTop=c;else{var k=fitPageWidthHeight(f,d,g,h);this.width=parseInt(k.x);this.height=parseInt(k.y);
this.bookLeft=e+(f-this.width)/2;this.bookTop=c+(d-this.height)/2}this.flipBookElement.css({width:this.width+"px",height:this.height+"px",left:this.bookLeft+"px",top:this.bookTop+"px"});this.setPageSize(g,h);this.updateDataWhenResize();this.refreshPages(b)},refreshPages:function(b){if(bookConfig.RightToLeft){var c=1-(this.currentOffset-this.startOffset-this.pageWidth)/this.unitOffset;var d=Math.max(1,Math.ceil(1-(this.currentOffset-this.startOffset+this.width)/this.unitOffset)-2);c=Math.min(bookConfig.totalPageCount,
Math.floor(c)+2)}else d=this.currentOffset-this.startOffset,c=d+this.width,d=Math.max(1,Math.floor(d/this.unitOffset)+1-2),c=Math.min(bookConfig.totalPageCount,Math.ceil(c/this.unitOffset)+2);for(var e=d;e<=c;e++){if(bookConfig.RightToLeft){var f=this.startOffset+this.unitOffset*(1-e)-this.currentOffset;var g=this.unitRotate*(1-e)-this.k*this.currentOffset}else f=this.startOffset+this.unitOffset*(e-1)-this.currentOffset,g=this.unitRotate*(e-1)-this.k*this.currentOffset;if(this.pageArray[e]){var h=
this.pageArray[e];h.refreshTransform({x:f,y:0},{x:0,y:g});if(!["flipping","flipped"].includes(b))h.onResize(this.pageWidth,this.pageHeight)}else h=new CardBookPage(this,e),h.initialize(this.pageWidth,this.pageHeight,{x:f,y:0},{x:0,y:g},e),this.pageArray[e]=h}if("flipping"!==b)for(b=0;b<this.pageArray.length;b++)(b<d||b>c)&&this.pageArray[b]&&(this.pageArray[b].remove(),this.pageArray[b]=void 0);if(b=this.pageArray[this.currentPageIndex])this.currentPage=b,this.showPage=b.pagesElement,$(".card-side").css({"z-index":3}),
...
this.pageArray[b].leaveSide()},addSearchHighlight:function(){var b=this.pageArray[this.currentPageIndex];b&&b.highlightSearch()},clearSearchHighlight:function(){for(var b=0;b<this.pageArray.length;b++)this.pageArray[b]&&this.pageArray[b].clearHighlight()},resetSlider:function(){for(var b=0;b<this.pageArray.length;b++)this.pageArray[b]&&this.pageArray[b].resetSlider()},onResize:function(){this.status_zoom=!1;this.endFlip();this.resizeBookAttachments();this.layoutPages("resize")},setPageSize:function(b,
c){bookConfig.isSingleBookFullWindowOnMobile&&(isPhone()||isPad())?(this.pageWidth=windowWidth,this.pageHeight=windowHeight):(b=fitPageWidthHeight(this.width,this.height,b,c),isPhone()||isPad(),this.pageWidth=parseInt(b.x),this.pageHeight=parseInt(b.y))},resizeBookAttachments:function(){this.bookAttachments.onResize(this.width,this.height)},getLeft:function(){return this.bookLeft},getTop:function(){return this.bookTop},getCssLeft:function(){return parseInt(this.flipBookElement.css("left"))},getCssTop:function(){return parseInt(this.flipBookElement.css("top"))},
getHeight:function(){return this.height},getWidth:function(){return this.width},getScale:function(){var b=this.pageArray[this.currentPageIndex];return b?b.pagesElement.parameters.toScale:1},updateDataWhenResize:function(){var b=bookConfig.totalPageCount;this.contentWidth=this.pageWidth*b+this.pageSpacing*(b-1);bookConfig.startingFromTheSide?(this.startOffset=0,this.scrollableWidth=this.contentWidth-this.width):(this.startOffset=this.width/2-this.pageWidth/2,this.scrollableWidth=this.contentWidth-
